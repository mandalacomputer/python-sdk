...
#: What a caller is told when a proxy abandoned their request.
#:
#: Written here rather than taken from the response, which is the one case where
#: the body is worth discarding: a 524 is generated by the edge, so it carries
#: an HTML error page or — when the request asked for JSON, as this client's
#: every request does — nothing at all. Both are useless, and the second is
#: worse than useless: it left ``str(e)`` reading ``HTTP 524`` and a caller with
#: nowhere to go, which is where the ceiling below cost real debugging time.
GATEWAY_TIMEOUT_MESSAGE = (
    "a proxy in front of the platform gave up waiting for it to answer. "
    "Nothing was cancelled: whatever this request started is still running, "
    "which is why the next call on the same computer may report the guest "
    "agent as busy. A foreground exec() ends this way after about two minutes "
    "however long a timeout it was given — the ceiling belongs to the proxy, "
    "not to the platform or to this client. Use start_exec() to run anything "
    "slower than that"
)
...
class _BaseTransport:
    ...
    def _error(resp: httpx.Response) -> APIError:
        ...
        if cls is GatewayTimeoutError:
            # Unconditionally, rather than only when the body was empty. A
            # proxy's error page is not this platform's error, so there is no
            # status where showing it beats saying what happened.
            return GatewayTimeoutError(GATEWAY_TIMEOUT_MESSAGE, status=resp.status_code, body=body)
