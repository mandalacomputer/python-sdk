...
class AsyncComputer(ComputerFields):
    ...
    async def ssh_access(self) -> SshAccess:
        ...
        """
        return SshAccess.from_api(await self._t.json_object("GET", _api.computer_ssh(self.id)))

    async def set_ssh_access(self, enabled: bool) -> SshAccess:
        ...
        data = await self._t.json_object("PUT", _api.computer_ssh(self.id), json=body)
        return SshAccess.from_api(data)

    async def secrets(self) -> SecretBindings:
    ...
    async def wait_for_browser_proxy(
        ...
        See :meth:`Computer.wait_for_browser_proxy`: this is its twin, and
    ...
    async def screenshot(
        ...
        **Shaping the picture.** ``region``, ``scale``, ``format`` and
        ...
        in that order to the same capture — the cheaper frame to hand a model::
            ...
            await c.screenshot(fresh=True, region=(0, 0, 640, 400), scale=0.5,
    ...
    async def retain_execution_output(
        self,
        execution_id: str,
        *,
        ...
        data = await self._t.bounded_json_object(
            "POST",
    ...
    async def result(self, result_id: str) -> RetainedResult:
    ...
    async def result_output(
        ...
        data, headers = await self._t.bounded_binary(
            "GET",
    ...
    async def delete_result(self, result_id: str) -> None:
        ...
        await self._t.bounded_binary(
            "DELETE", path, max_bytes=0, status=204, media=None, exact_bytes=0
        )
    ...
    async def download_artifact(self, artifact_id: str, *, max_bytes: int = 8388608) -> bytes:
        ...
        data = await self._t.bounded_json_object("GET", path, max_bytes=4096)
        ...
        content, _ = await self._t.bounded_binary(
            "GET",
            path + "/download",
        ...
        verified = verify_download(artifact, content)
    ...
    def events(
        ...
        **It reconnects, and it keeps your place.** Every event carries an
        ...
        :meth:`~mandala_computer.AsyncBackgroundCommand.poll` rather than to
        ...
        **Watching a directory.** ``file.changed`` is the one type that never
        ...
        The options are :meth:`Computer.events`'s, meaning for meaning.
        """
        ...
        * ``file.changed`` with no ``watch=``. A computer advertises that type
        ...
        def hello_hook(hello: Hello) -> None:
            ...
            if impossible is not None:
                ...
                # handshake. The iteration's cleanup does the socket.
                stream.close()
