...
@pytest.mark.parametrize("policy", [None, {"idempotent": 0}, {"idempotent": 2}])
async def test_terminal_newline_key_is_trimmed_before_native_header_serialization(policy, delays):
    wire = InMemoryProxyWire(
        [b"HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: 2\r\n\r\n[]"]
    )
    async with (
        httpx.AsyncClient(transport=wire, trust_env=False) as http,
        AsyncClient(
