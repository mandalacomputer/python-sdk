...
@respx.mock
async def test_wait_until_built_refreshes_after_the_timeout_boundary(
    client: mc.AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(mc._async_computer, "time", SimpleNamespace(monotonic=lambda: next(now)))
    monkeypatch.setattr(mc._async_computer, "asyncio", SimpleNamespace(sleep=no_sleep))
    computer = await mc.AsyncComputer(client._t, BUILDING).wait_until_built(timeout=1, poll=5)
